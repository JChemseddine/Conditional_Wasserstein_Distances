...
def parse_arguments():
    ...
    parser.add_argument("--ntrain", type=int, default=40000, help="Number of training steps")
...
def train_and_eval(args):
    ...
    for i in progress_bar:
        for k,x_full in enumerate(dataloader):
            ...
            xt = z + t.view(-1,1)*(x-z)
            xty = torch.cat((xt,y),1)
            net_inp = torch.cat((xty,t.unsqueeze(1).reshape(ntraj,1)),dim=1)   
            outp = gen(net_inp)
            optimizer.zero_grad()
            l = loss(outp, z-x)
            l.backward()
        ...
        with torch.no_grad():
            ...
            xy = torch.cat((x_val,y_val),dim=1)
            zy = torch.cat((z,y_val),dim=1)
            xt = z + t.view(-1,1)*(x_val-z)
            xty = torch.cat((xt,y_val),1)
            net_inp = torch.cat((xty,t.unsqueeze(1).reshape(val_samples,1)),dim=1)   
            outp = gen(net_inp)
            l = loss(outp, z-x_val)
            if l < min_loss:
                min_loss = l
                torch.save(gen.state_dict(), 'flow.pt')
    ...
    gen.load_state_dict(torch.load('flow.pt'))
    ...
    with torch.no_grad():
        for i in range(NO_y):
            ...
            if i < 10:
                make_image(gt_post.cpu().detach().data.numpy(), samples_model.cpu().detach().data.numpy(), 'img_fm_ot'+str(i), directory=directory)
    print(w2_mean)
    return w2_mean, 0
